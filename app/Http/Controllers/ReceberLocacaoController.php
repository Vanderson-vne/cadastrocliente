...
class ReceberLocacaoController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($user->hasAnyRole('Admin', 'Gerente', 'Caixa')) {
            ...
            $alugueisatrasados = $recibos->sum('taxa_adm');
            ...
            ->join('indice as in', 'r.idindice', '=', 'in.idindice')
            ->select('r.idrecibo', 'r.mes_ano', 'r.codigo', 'r.forma_pgto', 'r.dt_inicial', 'r.dt_final', 
            'r.taxa_adm','r.total_aluguel', 'l.idlocacao', 'i.nome as nomeinq', 'i.idinquilino as codinq', 
            'i.cpf_cnpj as cnpjcpfinq', 'i.telefone as foneinq', 'p.nome as nomepro', 'p.idproprietario', 
            'p.cpf_cnpj as cnpjcpfpro', 'p.telefone as fonepro', 'im.idimovel', 'im.codigo as codigoimo', 'im.endereco', 'im.bairro', 'im.cidade', 'in.nome as nomeind', 'r.dt_inicial', 'r.dt_final', 'r.contador_aluguel', 'r.reajuste', 'l.reajuste_sobre', 'r.dt_vencimento', 'r.dt_pagamento', 'r.taxa_adm', 'r.liquido', 'r.forma_pgto')
            ->where('r.dt_pagamento', '=', '"2000-01-01 00:00:01"')->orWhereNull('r.dt_pagamento')
            ->whereBetween('r.dt_vencimento', [$dtInicial, $dtFinal])
            ->orderBy('r.dt_vencimento', 'desc')
            ->get();

            $alugueisavencer = $recibos->sum('taxa_adm');
            ...
            $alugueisavencerdia = $recibos->sum('taxa_adm');
            ...
            ->join('imovel as im', 'r.idimovel', '=', 'im.idimovel')
            ->join('indice as in', 'r.idindice', '=', 'in.idindice')
            ->select('r.taxa_adm','r.idrecibo', 'r.mes_ano', 'l.idlocacao', 'i.nome as nomeinq',
            ...
            $alugueispagodia = $recibos->sum('taxa_adm');

           // dd($dtPgto,$dtPgto1,$alugueispagodia);
            ...
            if ($request) {
               ...
               // dd($recibos,$recibosbx);
                ...
                return view('financeiro.receberLocacao.index', [
                    "recibos" => $recibos,
                    "recibosbx" => $recibosbx,
                    "empresas" => $empresas,
